...
const InsightDashboard = () => {
      ...
      <div className="bg-white rounded-xl shadow-lg p-6 border border-gray-100 mb-8">
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-3">
          {toShow.map((company, idx) => (
            <div
              key={idx}
              className="bg-gray-50 p-3 rounded-lg border border-gray-200 hover:border-indigo-400 transition-colors"
            >
  ...
  const lineChartData =
    !isSummer && activeInsights?.offersVsDate
      ? {
          labels:
    ...
    datasets: [
      {
        ...
        data: Object.values(activeInsights?.departmentStats || {}).map((e) =>
          isSummer
            ? (e?.internshipPercentage ?? 0)
            : (e?.placementPercentage ?? 0),
        ),
...
const rechartsDeptData = Object.entries(activeInsights?.departmentStats || {}).map(([name, stats]) => ({
  ...
  const stipendDoughnutData =
    isSummer && activeInsights?.stipendBuckets
      ? {
  ...
  const getDeptData = () => {
    ...
    const match = Object.keys(stats).find(
      (key) =>
        key.trim().toUpperCase() === selectedDepartment.trim().toUpperCase(),
    );
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
            ...
            <StatCard
              ...
              value={
                isSummer
                  ? deptData.doubleOffers || deptData.multipleOffers || 0
            ...
            {isSummer ? (
              <>
                <StatCard
                  ...
                  value={
                    deptData.avgStipend ? `₹${deptData.avgStipend}K/mo` : "N/A"
                  }
                  subtitle="Per month"
                  color="green"
                />
                <StatCard
                  ...
                  value={
                    deptData.highestStipend
                      ? `₹${deptData.highestStipend}K/mo`
                      : "N/A"
                  }
                  subtitle="Per month"
                  color="blue"
                ...
                <StatCard
                  icon={IndianRupee}
                  title="Lowest Stipend"
                  value={
                    deptData.lowestStipend
                      ? `₹${deptData.lowestStipend}K/mo`
                      : "N/A"
                  }
                  subtitle="Per month"
                  color="yellow"
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 mb-8">
            {isSummer && deptData.stipendBuckets && (
              <ChartCard title="Stipend Distribution" icon={PieChart}>
                <Doughnut
                  data={{
                    labels: Object.keys(deptData.stipendBuckets || {}),
                    datasets: [
                      {
                        data: Object.values(deptData.stipendBuckets || {}),
                        backgroundColor: stipendColors,
                        borderWidth: 0,
                        cutout: "60%",
                      },
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 mb-8">
              <ChartCard title="Top 5 Companies (Offer Count)" icon={Building}>
                <Bar
                  data={{
                    labels:
                      deptData.topCompaniesByCount?.map((i) => i.company) || [],
                    datasets: [
                      {
                        label: "Offers",
                        data:
                          deptData.topCompaniesByCount?.map((i) => i.count) ||
                          [],
              ...
              <ChartCard title="Top 5 Companies (Highest CTC)" icon={Award}>
                <Bar
                  data={{
                    labels:
                      deptData.topCompaniesByAvgCTC?.map((i) => i.company) ||
                      [],
                    datasets: [
                      {
                        label: "CTC (₹LPA)",
                        data:
                          deptData.topCompaniesByAvgCTC?.map((i) => i.avgCTC) ||
                          [],
              ...
              {[
                {
                  ...
                  data: deptData.jobTypeDist,
                  color: "green",
                },
                {
                  title: "Category Distribution",
                  data: deptData.categoryDist,
                  color: "indigo",
                },
                {
                  title: "Industry Sector",
                  data: deptData.industryDist,
                  color: "orange",
                },
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
          ...
          {isSummer ? (
            <>
              ...
              <StatCard
                ...
                title="Average Stipend"
                value={
                  activeInsights.avgStipend
                    ? `₹${activeInsights.avgStipend}K/mo`
                    : "N/A"
                }
                subtitle="Per month"
                color="green"
              ...
              <StatCard
                ...
                title="Highest Stipend"
                value={
                  activeInsights.highestStipend
                    ? `₹${activeInsights.highestStipend}K/mo`
                    : "N/A"
                }
                subtitle="Per month"
                color="blue"
              ...
              <StatCard
                ...
                title="Lowest Stipend"
                value={
                  activeInsights.lowestStipend
                    ? `₹${activeInsights.lowestStipend}K/mo`
                    : "N/A"
                }
                subtitle="Per month"
                color="yellow"
            ...
            <>
              ...
              <StatCard
                ...
                title="Median CTC"
                value={
                  activeInsights.medianCTC
                    ? `₹${activeInsights.medianCTC}L`
                    : "N/A"
                }
                subtitle="Per annum"
                color="green"
        ...
        {!isSummer && (
          <>
            <select
              ...
              className="bg-white border border-gray-300 rounded-lg px-4 py-2 text-sm font-medium text-gray-700 hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-indigo-500 mb-5"
            >
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 mb-8">
          {isSummer && summerLineChartData && (
            <ChartCard title="Internship Offer Timeline" icon={Calendar}>
              <Line data={summerLineChartData} options={chartOptionsdatewise} />
            </ChartCard>
          )}
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 mb-8">
          {isSummer && stipendDoughnutData && (
            <ChartCard title="Stipend Distribution" icon={PieChart}>
              <Doughnut data={stipendDoughnutData} options={chartOptions} />
            </ChartCard>
          )}
        ...
        {isSummer && activeInsights.topCompanies?.length > 0 && (
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 mb-8">
            <ChartCard title="Top Companies by Offers" icon={Building}>
              <Bar
                data={{
                  labels: activeInsights.topCompanies.map((i) => i.company),
                  datasets: [
                    {
                      label: "Offers",
                      data: activeInsights.topCompanies.map((i) => i.count),
                      backgroundColor: "rgba(168,85,247,0.8)",
                      borderRadius: 8,
                    },
                  ],
            ...
            {activeInsights.topCompaniesByStipend?.length > 0 && (
              <ChartCard
                title="Top Companies by Stipend (K/mo)"
                icon={TrendingUp}
              >
                <Bar
                  data={{
                    labels: activeInsights.topCompaniesByStipend.map(
                      (i) => i.company,
                    ),
                    datasets: [
                      {
                        label: "Max Stipend",
                        data: activeInsights.topCompaniesByStipend.map(
                          (i) => i.maxStipend,
                        ),
        ...
        {!isSummer && (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
            {[
              {
                title: "Job Type Distribution",
                data: activeInsights.jobTypeDist,
                dotColor: "indigo",
              },
              {
                title: "Category Distribution",
                data: activeInsights.categoryDist,
                dotColor: "green",
              },
              {
                title: "Industry Sectors",
                data: activeInsights.sectorDist,
                dotColor: "orange",
              },
                  ...
                  {Object.entries(data || {}).map(([key, count]) => (
                    ...
                    >
                      ...
                      <div className="flex items-center space-x-2">
                        <div
                          className={`w-2 h-2 bg-${dotColor}-500 rounded-full`}
                        ></div>
                        <span className={`font-semibold text-${dotColor}-600`}>
                          {count}
                        </span>
                      </div>
                    </div>
