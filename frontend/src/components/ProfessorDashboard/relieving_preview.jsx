...
const NOCPreview = ({ noc, onClose }) => {
  const formatDate = (date) =>
    date
      ...
      : "N/A";
  const year = noc.year || "N/A";
  const semester = noc.semester || "N/A";
  const baseURL = import.meta.env.REACT_APP_BASE_URL || "";
  const isOffCampus = noc.internshipMode === "Off-Campus";
  const isOwnStartup = noc.internshipMode === "Own Startup";
  const needsExtra =
    (noc.course === "M.Tech" ||
      (noc.course === "B.Tech" && noc.year === "4th")) &&
    noc.internshipMode !== "On-Campus";
  ...
  const renderDocumentStatus = (label, uploaded) => (
    <span
      className={`flex items-center gap-2 px-3 py-1 rounded-full text-sm font-medium ${uploaded ? "bg-green-100 text-green-700 cursor-pointer hover:bg-green-200" : "bg-gray-200 text-gray-600"}`}
        ...
        <div className="p-8 space-y-8">
          ...
          <div className="bg-gradient-to-br from-blue-50 to-white rounded-xl p-6 border border-blue-100 shadow-sm">
            <h2 className="text-2xl font-bold text-gray-800 mb-6 flex items-center gap-2">
              <span className="p-2 bg-custom-blue rounded-lg text-white">
                👤
              </span>{" "}
              Student Details
            </h2>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              <DetailItem
                label="Year & Semester"
                value={`${noc.year || ""} - ${noc.semester || ""}`}
                colSpan={true}
              />
              <DetailItem
                label="Respondent Email"
                value={noc.respondentEmail}
                colSpan={true}
              />
            </div>
          </div>
          ...
          <div className="bg-gradient-to-br from-blue-50 to-white rounded-xl p-6 border border-blue-100 shadow-sm">
            <h2 className="text-2xl font-bold text-gray-800 mb-6 flex items-center gap-2">
              ...
              {isFTE ? "FTE Details" : "Relieving Details"}
            </h2>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              {isFTE ? (
                ...
                <>
                  ...
                  <DetailItem
                    label="From"
                    value={formatDate(noc.internshipFrom)}
                  />
            ...
            <>
              ...
              <div className="bg-gradient-to-br from-blue-50 to-white rounded-xl p-6 border border-blue-100 shadow-sm">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  ...
                  <DetailItem
                    label="Designation"
                    value={noc.contactPersonDesignation}
                  />
              ...
              {needsExtra && (
                <>
                  ...
                  <div className="bg-gradient-to-br from-blue-50 to-white rounded-xl p-6 border border-blue-100 shadow-sm">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                      <DetailItem
                        label="Minimum 3 Years Old"
                        value={noc.companyminAgeis3 === "Yes" ? "Yes" : "No"}
                      />
                      <DetailItem
                        label="Turnover Last FY (Crore)"
                        value={noc.companyTurnoverLastFY}
                      />
                      <DetailItem
                        label="Company Type"
                        value={noc.companyType}
                      />
                      <DetailItem
                  ...
                  <DocumentsSection
                    documents={[
                      { label: "Offer Letter", uploaded: noc.offerLetter },
                      {
                        ...
                        uploaded: noc.turnoverReport,
                      },
                      {
                        label: "Mail Screenshot",
                        uploaded: noc.mailScreenshot,
                      },
                    ]}
          ...
          {isOwnStartup && needsExtra && (
            <>
              ...
              <div className="bg-gradient-to-br from-blue-50 to-white rounded-xl p-6 border border-blue-100 shadow-sm">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  <DetailItem
                    ...
                    value={formatDate(noc.startupEstablishedDate)}
                  />
                  <DetailItem
                    label="Business Registration Type"
                    value={noc.businessRegistrationType}
                  />
                  <DetailItem label="PAN No" value={noc.panNo} />
                  <DetailItem label="GST No" value={noc.gstNo} />
                  <DetailItem
                    label="MSME Registration Category"
                    value={noc.MSMERegistrationCategory}
                    colSpan={true}
                  />
                </div>
              </div>
              ...
              <BankDetailsSection
                ...
                bankDetails={noc.startupBankDetails}
              />
              {/* Documents */}
              <DocumentsSection
                documents={[
                  {
                    label: "Startup India Recognition Certificate",
                    uploaded: noc.startupIndiaRecognitionCertificate,
                  },
                ]}
                renderStatus={renderDocumentStatus}
              />
          ...
          {needsExtra && (
            <>
              ...
              <div className="bg-gradient-to-br from-blue-50 to-white rounded-xl p-6 border border-blue-100 shadow-sm">
                ...
                <div
                  className={`p-4 rounded-lg ${noc.signature ? "bg-green-50 border border-green-200" : "bg-gray-50 border border-gray-200"}`}
                >
                  {noc.signature ? (
      ...
      {noc.nocLetter && (
        <div className="bg-gradient-to-br from-blue-50 to-white rounded-xl p-6 border border-blue-100 shadow-sm">
          <h2 className="text-2xl font-bold text-gray-800 mb-4 text-center">
            Final Relieving Letter
          </h2>
          <div className="relative w-full h-[600px] bg-gray-100 rounded-lg overflow-hidden border-2 border-blue-200">
            <iframe
              src={`${baseURL}/${noc.nocLetter}`}
              width="100%"
              height="100%"
              className="rounded-lg shadow-inner"
              title="Relieving Letter PDF"
            />
          </div>
        </div>
      )}
  ...
  <div className="bg-gradient-to-br from-blue-50 to-white rounded-xl p-6 border border-blue-100 shadow-sm">
    ...
    <ul className="space-y-3">
      {documents.map((doc, index) => (
        <li
          key={index}
          className={`flex items-center justify-between p-4 rounded-lg ${doc.uploaded ? "bg-green-50 border border-green-200" : "bg-gray-50 border border-gray-200"}`}
        >
