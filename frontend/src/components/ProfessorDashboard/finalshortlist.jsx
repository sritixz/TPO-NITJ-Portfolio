...
const FinalShortlistStudents = ({ jobId, onClose }) => {
    ...
    const fetchEligibleStudents = async () => {
      ...
      try {
        ...
        const updatedStudents = eligibleStudents
          .map((student) => ({
            ...student,
            initiallyShortlisted: student.shortlisted,
  ...
  const handleInputChange = (index, field, value) => {
    const updatedStudents = [...students];
    if (!updatedStudents[index].initiallyShortlisted || !updatedStudents[index].shortlisted) {
  ...
  const handleInitialSubmit = () => {
    const hasChanges = students.some((student) => student.shortlisted !== student.initiallyShortlisted);
    const shortlistedStudents = students.filter((student) => student.shortlisted);
    ...
    for (const student of shortlistedStudents) {
      ...
      if (!student.job_type) {
        toast.error('Please select Job Type for all newly shortlisted students');
        return;
      }
      if (student.job_type === 'Intern') {
        if (!student.stipend || !student.intern_duration) {
          toast.error('Please fill in Stipend and Internship Duration for Intern students');
          return;
        }
      } else if (student.job_type === 'Intern+PPO' || student.job_type === 'Intern+FTE') {
        if (!student.ctc || !student.stipend || !student.intern_duration) {
          toast.error('Please fill in CTC, Stipend, and Internship Duration for Intern+PPO/FTE students');
          return;
        }
      ...
      if (!student.job_role) {
        toast.error('Please fill in Job Role for all newly shortlisted students');
        return;
      }
  ...
  const handleFinalSubmit = async () => {
    ...
    try {
      const updatedStudents = students
        .filter((student) => student.shortlisted !== student.initiallyShortlisted)
        .map(({ studentId, shortlisted, initiallyShortlisted, job_type, job_role, ctc, stipend, intern_duration }) => ({
          studentId,
          ...
          stipend: shortlisted && job_type.includes('Intern') ? stipend : undefined,
          internduration: shortlisted && job_type.includes('Intern') ? intern_duration : undefined,
        }));
  ...
  const filteredStudents = students.filter(
    (student) =>
      starredFields.some((field) =>
        student[field.fieldName]?.toString().toLowerCase().includes(searchQuery.toLowerCase())
      ) || student.name.toLowerCase().includes(searchQuery.toLowerCase())
  );

  // Determine if Stipend and Intern Duration columns should be shown
  const showInternColumns = students.some(
    (student) => student.job_type === 'Intern' || student.job_type === 'Intern+PPO' || student.job_type === 'Intern+FTE'
  );

  const renderInputFields = (student, index) => {
    const isIntern = student.job_type === 'Intern';
    const isInternPlus = student.job_type === 'Intern+PPO' || student.job_type === 'Intern+FTE';
      ...
      <>
        <td className="px-4 py-2 border text-center">
          <select
            ...
            onChange={(e) => handleInputChange(students.findIndex((s) => s.studentId === student.studentId), 'job_type', e.target.value)}
          ...
          <>
            <td className="px-4 py-2 border text-center">
              <input
                ...
                onChange={(e) => handleInputChange(students.findIndex((s) => s.studentId === student.studentId), 'stipend', e.target.value)}
                className="w-full px-2 py-1 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 disabled:bg-gray-100"
        ...
        {(isInternPlus || isFTE) && (
          <td className="px-4 py-2 border text-center">
            <input
              type="text"
              value={student.ctc}
              onChange={(e) => handleInputChange(students.findIndex((s) => s.studentId === student.studentId), 'ctc', e.target.value)}
              className="w-full px-2 py-1 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 disabled:bg-gray-100"
        ...
        <td className="px-4 py-2 border text-center">
          <input
            ...
            onChange={(e) => handleInputChange(students.findIndex((s) => s.studentId === student.studentId), 'job_role', e.target.value)}
            className="w-full px-2 py-1 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 disabled:bg-gray-100"
  ...
  const renderConfirmFields = (student) => {
    ...
    return (
      <>
        <td className="px-4 py-2 border text-center">{student.job_type || 'N/A'}</td>
        {showInternColumns && (isIntern || isInternPlus) && (
          <>
            <td className="px-4 py-2 border text-center">{student.stipend || 'N/A'}</td>
            <td className="px-4 py-2 border text-center">{student.intern_duration || 'N/A'}</td>
          </>
        )}
        {showInternColumns && !(isIntern || isInternPlus) && (
          <>
        ...
        {(isInternPlus || isFTE) && (
          <td className="px-4 py-2 border text-center">{student.ctc || 'N/A'}</td>
        )}
        {!(isInternPlus || isFTE) && (
          <td className="px-4 py-2 border text-center"></td>
        )}
        <td className="px-4 py-2 border text-center">{student.job_role || 'N/A'}</td>
      </>
    );
  };
            ...
            <table className="w-full text-sm text-left text-gray-500 border-collapse">
                ...
                <tr>
                  ...
                  {showInternColumns && (
                    <>
                      ...
                      <th className="px-4 py-2 border text-center">Intern Duration</th>
                    </>
                  )}
              ...
              <tbody>
                {filteredStudents.map((student, index) => (
                  <tr key={index} className={`bg-white border-b ${student.initiallyShortlisted ? 'bg-gray-50' : ''}`}>
                    ...
                    <td className="px-4 py-2 border text-center">
                      <input
                        ...
                        onChange={() => handleCheckboxChange(students.findIndex((s) => s.studentId === student.studentId))}
                      />
                    </td>
                  </tr>
                ))}
        ...
        {showConfirmModal && (
          <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
            <div className="bg-white p-6 rounded-lg shadow-lg max-w-4xl w-full max-h-[80vh] overflow-y-auto">
              <h2 className="text-xl font-bold mb-4">Confirm Shortlist Changes</h2>
              <p className="mb-4">Please review the changes to the shortlist before submitting:</p>
              {students.filter((student) => student.shortlisted || student.initiallyShortlisted).length > 0 ? (
                    ...
                    {students
                      ...
                      .map((student, index) => (
                        <tr key={index} className="bg-white border-b">
                          ...
                          <td className="px-4 py-2 border text-center">
                            {student.shortlisted ? 'Shortlisted' : 'Unshortlisted'}
                          </td>
                        </tr>
                      ))}
                  </tbody>
                ...
                <p className="text-gray-600 mb-4">No students are currently shortlisted.</p>
              )}
