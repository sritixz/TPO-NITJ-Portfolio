...
const StudentDocumentManager = () => {
  ...
  const fetchDocuments = async () => {
    try {
      setLoading(true);
      const response = await axios.get(`${import.meta.env.REACT_APP_BASE_URL}/admin/student-documents/get`, {
  ...
  const handleSave = async () => {
    try {
      const url = editDocument?._id
        ? `${import.meta.env.REACT_APP_BASE_URL}/admin/student-documents/update/${editDocument._id}`
        : `${import.meta.env.REACT_APP_BASE_URL}/admin/student-documents/add`;
  ...
  const handleDeleteDocuments = async () => {
    try {
      ...
      await axios.delete(`${import.meta.env.REACT_APP_BASE_URL}/admin/student-documents/delete`, {
  ...
  const handleExportJSON = () => {
    try {
      ...
      link.download = `student_document_${timestamp}.json`;
      ...
      if (dataToExport.length === 0) {
        ...
        toast.success(`Exported ${dataToExport.length} student documents(s) to JSON`);
      }
    } catch (error) {
