...
const OutsiderDocumentManager = () => {
  ...
  const fetchDocuments = async () => {
    try {
      setLoading(true);
      const response = await axios.get(`${import.meta.env.REACT_APP_BASE_URL}/admin/outsider-documents/get`, {
  ...
  const handleSave = async () => {
    try {
      const url = editDocument?._id
        ? `${import.meta.env.REACT_APP_BASE_URL}/admin/outsider-documents/update/${editDocument._id}`
        : `${import.meta.env.REACT_APP_BASE_URL}/admin/outsider-documents/add`;
  ...
  const handleDeleteDocuments = async () => {
    try {
      ...
      await axios.delete(`${import.meta.env.REACT_APP_BASE_URL}/admin/outsider-documents/delete`, {
  ...
  const handleExportJSON = () => {
    try {
      ...
      link.download = `outsider_document_${timestamp}.json`;
      ...
      if (dataToExport.length === 0) {
        ...
        toast.success(`Exported ${dataToExport.length} outsider document(s) to JSON`);
      }
    } catch (error) {
...
export default OutsiderDocumentManager;
