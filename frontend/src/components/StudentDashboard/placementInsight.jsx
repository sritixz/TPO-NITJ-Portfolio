...
const getAbbreviation = (deptName) => {
  ...
  while (allExistingAbbreviations.includes(finalAbbreviation)) {
    if (words.length >= 2) {
      ...
      finalAbbreviation =
        abbreviation + words[0].substring(1, 1 + counter).toUpperCase();
    } else {
      // Add numeric suffix
    ...
    if (counter > 10) {
      finalAbbreviation =
        abbreviation + Math.random().toString(36).substring(2, 4).toUpperCase();
      break;
    }
  }
...
const PlacementInsights = () => {
  ...
  const handleDownloadExcel = async () => {
    ...
    const stats = [
      ...
      w[("Highest CTC (₹LPA)", insights.highestCTC || "N/A")],
        ...
        callbacks: {
          title: (context) => {
            ...
            const fullNames = Object.keys(
              insights.placementsByDepartment || {},
            );
            const fullName = fullNames.find(
              (name) => getAbbreviation(name) === abbreviation,
            );
    ...
    scales: {
      x: {
        ...
        ticks: {
          font: {
            size:
              window.innerWidth < 640 ? 8 : window.innerWidth < 768 ? 9 : 11,
            weight: "500",
          },
          autoSkip: false,
  ...
  const departmentBarData = {
    labels: Object.keys(insights.departmentStats || {}).map((dept) =>
      getAbbreviation(dept),
    ),
    datasets: [
      {
        label: "Placements",
        ...
        <div className="min-h-screen bg-white flex items-center justify-center">
          <div className="text-center">
            <div className="bg-white rounded-lg shadow-lg p-8">
              ...
              <p className="text-sm text-gray-500">
                for the selected department or no placement yet
              </p>
            </div>
          </div>
        </div>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 mb-8">
          ...
          <ChartCard
            title={
              selectedCourse === "ALL"
                ? "Course-wise Placement Percentage"
