...
export default function TinyMCEEditor({ experience, onClose }) {
  const editorRef = useRef(null);
  const [isSaving, setIsSaving] = useState(false);
  const [editorData, setEditorData] = useState(experience);
  ...
  const handleSave = async () => {
    ...
    const tempcontent = editorRef.current.getContent();
    await submitExperience(tempcontent);
  };

  const submitExperience = async (tempcontent) => {
    setIsSaving(true);
    try {
      // Extract <h1> title and remaining content
      const tempDiv = document.createElement("div");
      tempDiv.innerHTML = tempcontent;
      const titleElement = tempDiv.querySelector("h1");
      ...
      await axios.post(endpoint, { title, content }, { withCredentials: true });

      toast.success(
        editorData ? "Experience updated successfully!" : "Experience submitted successfully!"
      );
  ...
  return (
    <div className="min-h-screen bg-gray-50 py-2 px-4 sm:px-6 lg:px-8">
      <div className="max-w-7xl mx-auto">
          ...
          <div className="grid md:grid-cols-3 gap-4">
            <div className="bg-gradient-to-br from-blue-50 to-indigo-50 p-4 rounded-xl border border-blue-100">
              ...
              <p className="text-sm text-gray-600">
                Share specific details about the interview process, questions asked, and preparation
                strategies.
              </p>
            </div>
            <div className="bg-gradient-to-br from-purple-50 to-pink-50 p-4 rounded-xl border border-purple-100">
              <h4 className="font-semibold text-gray-800 mb-2">Be Honest</h4>
              <p className="text-sm text-gray-600">
                Include both challenges you faced and how you overcame them to help others learn.
              </p>
            </div>
            <div className="bg-gradient-to-br from-green-50 to-teal-50 p-4 rounded-xl border border-green-100">
              <h4 className="font-semibold text-gray-800 mb-2">Be Helpful</h4>
              <p className="text-sm text-gray-600">
                Provide actionable advice and resources that helped you succeed in your journey.
              </p>
            </div>
          </div>
        </div>
        ...
        {/* Editor Container */}
        <div className="bg-white rounded-2xl shadow-md overflow-hidden border border-gray-200 mt-8">
          <div className="p-6">
            <Editor
              // NEW: Load from local self-hosted script (no API key needed)
              tinymceScriptSrc="/tinymce/tinymce.min.js"
              // NEW: GPL license for self-hosting (free community edition)
              licenseKey="gpl"
              onInit={(evt, editor) => (editorRef.current = editor)}
              initialValue={
                editorData
                  ? `<h1>${editorData.title}</h1>${editorData.content}`
                  : "<h1>Write Title Here...</h1><p>Start sharing your experience...</p>"
              }
              init={{
                height: 550,
                menubar: true,
