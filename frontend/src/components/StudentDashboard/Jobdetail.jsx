...
import { FaArrowLeft } from "react-icons/fa";
import ApplicationForm from "./applicationform";
import StudentSubmissions from "./viewapplicationform";
import { FontAwesomeIcon } from "@fortawesome/react-fontawesome";
import { Calendar, Users, FileText, Award } from 'lucide-react';
import {
  Clock,
...
const Jobdetail = () => {
  ...
  <div className="fixed inset-0 flex items-center justify-center bg-black bg-opacity-40 z-50">
    <div className="relative p-6 bg-white rounded-xl shadow-lg w-[90%] max-w-3xl max-h-[80vh] overflow-y-auto">
      {/* Close button */}
      <button
        onClick={() => setDescription(false)}
      ...
      {/* Title */}
      <h2 className="text-xl text-custom-blue text-center font-semibold mb-4">
        Description
      </h2>
  ...
  <div className="py-8 px-4 sm:px-6 lg:px-8">
    <div className="max-w-3xl mx-auto">
      {!jobDetails?.Hiring_Workflow || jobDetails?.Hiring_Workflow.length === 0 ? (
        // No hiring flow available
        <div className="text-center py-12">
          <div className="bg-gray-50 border border-gray-200 rounded-lg p-8">
            ...
            <h3 className="text-lg font-medium text-gray-900 mb-2">Hiring Flow Not Available</h3>
            <p className="text-gray-600">
              Hiring flow not mentioned. Please use job description attachment or contact your PR.
            </p>
          </div>
        </div>
      ) : (
        // Existing hiring flow timeline
        <div className="relative">
          <div className="absolute left-5 inset-y-0 w-0.5 bg-gradient-to-b from-blue-400 via-indigo-500 to-purple-600 rounded-full"></div>
          {jobDetails?.Hiring_Workflow?.map((step, index) => {
            const theme = getStepTheme(step.step_type);
...
{step.step_announcements && step.step_announcements.length > 0 && (
  <div className="mt-6 space-y-4">
    ...
    eligibilityCriteria: (
      ...
      <button
        className="px-4 sm:px-5 py-2 rounded-lg font-semibold text-white bg-blue-500 cursor-not-allowed"
        disabled
      >
        Applied
      </button>
