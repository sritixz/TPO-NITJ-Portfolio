...
import { useSelector } from 'react-redux';
import axios from 'axios';
import { FaPlus, FaEye, FaDownload, FaEdit, FaLock, FaTrash } from 'react-icons/fa';
import GenerateNOC from './generate-noc';
...
const NOC = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    const url = editingId ? `/noc/${editingId}` : `/noc`;
  ...
  const handleLock = async (id) => {
    try {
      await axios.put(`${import.meta.env.REACT_APP_BASE_URL}/noc/lock/${id}`, {}, { withCredentials: true });
      setNocs(prev => prev.map(n => n._id === id ? { ...n, locked: true } : n));
      showToast('NOC locked successfully!', 'success');
    } catch (error) {
      console.error('Error locking NOC:', error);
      showToast('Failed to lock NOC. Try again!', 'error');
    }
  };

  // Handle delete
  const handleDelete = async (id) => {
    if (!window.confirm('Are you sure you want to delete this NOC? This action cannot be undone.')) return;
    try {
      await axios.delete(`${import.meta.env.REACT_APP_BASE_URL}/noc/${id}`, { withCredentials: true });
      setNocs(prev => prev.filter(n => n._id !== id));
      showToast('NOC deleted successfully!', 'success');
    } catch (error) {
      console.error('Error deleting NOC:', error);
      showToast('Failed to delete NOC. Try again!', 'error');
    }
  };
  ...
  : 'bg-green-100 text-green-800'
              ...
              <p className="bg-custom-blue/10 rounded-lg p-1 text-custom-blue text-xs font-semibold inline-block"># {noc.nocId}</p>
              <div className="flex flex-wrap gap-2 mt-4">
                {!noc.locked ? (
                  <>
                    <button
