...
const NOC = () => {
  ...
  useEffect(() => {
    setLoading(true);
    axios
      .get(`${import.meta.env.REACT_APP_BASE_URL}/noc`, {
        withCredentials: true,
      })
      .then((response) => {
        setNocs(response.data);
...
if (formData.purpose === "FTE") {
  ...
  const handleLock = async (id) => {
    try {
      await axios.put(
        `${import.meta.env.REACT_APP_BASE_URL}/noc/lock/${id}`,
        {},
        { withCredentials: true },
      );
      setNocs((prev) =>
  ...
  const handleDelete = async (id) => {
    if (
      !window.confirm(
        "Are you sure you want to delete this NOC? This action cannot be undone.",
      )
    )
      return;
    try {
      await axios.delete(`${import.meta.env.REACT_APP_BASE_URL}/noc/${id}`, {
        withCredentials: true,
      });
  ...
  const renderNOCList = () => (
    <div className="space-y-6 animate-fade-in">
      <div className="flex justify-between items-center mb-8 gap-4">
        <h2 className="text-3xl font-bold flex items-center space-x-3 text-gray-900">
          <span>
            No Objection <span className="text-custom-blue">Certificate</span>
          </span>
        </h2>
        <div className="flex items-center gap-4 flex-1 justify-end">
          <button
        ...
        <div className="bg-white shadow-lg rounded-lg p-8 mb-8 border border-gray-200 relative">
          <div className="flex justify-between items-center mb-6">
            <h2 className="text-xl font-semibold text-gray-700">
              Apply for <span className="text-custom-blue">NOC</span>
            </h2>
            <button
              onClick={() => {
                setShowForm(false);
