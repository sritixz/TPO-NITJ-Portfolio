...
const InternshipDetailsDownload = ({ internship }) => {
  ...
  const downloadPDF = async () => {
      ...
      <Document>
        <Page size="A4" style={styles.page}>
          ...
          <View style={styles.section}>
            <Text style={styles.sectionTitle}>Internship Information</Text>
    ...
    const blob = await pdf(InternshipPDF).toBlob();
    const url = URL.createObjectURL(blob);
    const link = document.createElement('a');
    link.href = url;
    link.download = `${internship.company_name || 'internship'}-details-${Date.now()}.pdf`;
...
export default RecentInternship;