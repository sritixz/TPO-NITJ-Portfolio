...
const StudentConnect = () => {
  ...
  const [filterOptions, setFilterOptions] = useState({
    ...
    degrees: ["B.Tech", "M.Tech", "MBA", "M.Sc"],
    departments: []
  });
  ...
  const fetchReportData = async (currentFilters) => {
    ...
    try {
      ...
      const response = await axios.get(`${import.meta.env.REACT_APP_BASE_URL}/pplacementReport/student-connect?${queryParams.toString()}`, { withCredentials: true });
    ...
    <div className="font-sans p-1 min-h-screen">
      ...
      <div className="bg-white rounded-lg shadow-lg p-6">
        {/* <div className="flex flex-col md:flex-row justify-between items-center mb-6 border-b border-gray-200 pb-4">
          <h2 className="text-3xl font-bold mb-4 md:mb-0">Student <span className='text-custom-blue'>Connect</span></h2>
        </div> */}
        <div className="flex flex-col md:flex-row justify-between items-center mb-6 border-b border-gray-200 pb-4">
  <div className="flex items-center gap-4">
    <h2 className="text-3xl font-bold">
      Student <span className="text-custom-blue">Connect</span>
    </h2>
    <Network className="w-8 h-8 text-custom-blue" />
  </div>
</div>
          ...
          <table className="w-full min-w-[1000px]">
            <thead>
              <tr className="bg-custom-blue text-white">
                ...
                <th className="py-3 px-4 text-center font-semibold border-b-2 bg-custom-blue whitespace-nowrap w-[5%]">Batch</th>
                <th className="py-3 px-4 text-center font-semibold border-b-2 bg-custom-blue whitespace-nowrap w-[5%]">Course</th>
                ...
                <th className="py-3 px-4 text-center font-semibold border-b-2 bg-custom-blue whitespace-nowrap w-[5%]">LinkedIn</th>
              </tr>
            </thead>
            <tbody>
              {currentRows.length > 0 ? (
                currentRows.map((item, index) => (
                  ...
                  >
                    ...
                    <td className="py-3 px-4 border-b border-gray-200 text-center">{item.batch}</td>
                    <td className="py-3 px-4 border-b border-gray-200 text-center">{item.degree}</td>
                    ...
                    <td className="py-3 px-4 border-b border-gray-200 text-center">{item.linkedin ? (
    <a href={item.linkedin} target="_blank" rel="noopener noreferrer" className="inline-flex items-center justify-center text-custom-blue hover:text-blue-800 transition-colors" title="View LinkedIn Profile">
      <Linkedin size={20} />
    </a>
  ) : (
