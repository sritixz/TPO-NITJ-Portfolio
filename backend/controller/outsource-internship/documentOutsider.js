...
const uploadDir = path.join(process.cwd(), 'uploads', 'documentsOutsider');
...
// Get all documents
export const getDocumentsOutsider = async (req, res) => {
  try {
    const documents = await DocumentOutsider.find();
    res.status(200).json({
      success: true,
      data: documents,
    });
...
export const addDocumentOutsider = [
  ...
  async (req, res) => {
    try {
      ...
      const document_link = `/uploads/documentsOutsider/${req.file.filename}`;
      console.log('Saving file to:', path.join(uploadDir, req.file.filename)); // Debug log

      const newDocument = await DocumentOutsider.create({
        document_name,
        document_link,
      });
...
export const updateDocumentOutsider = [
  upload.single('document_file'),
  async (req, res) => {
    try {
      ...
      const existingDocument = await DocumentOutsider.findById(documentId);
      if (!existingDocument) {
      ...
      if (req.file) {
        ...
        updateData.document_link = `/uploads/documentsOutsider/${req.file.filename}`;
        console.log('Updating file to:', path.join(uploadDir, req.file.filename));
      }

      const updatedDocument = await DocumentOutsider.findByIdAndUpdate(
        documentId,
        updateData,
...
// Delete document
export const deleteDocumentOutsider = async (req, res) => {
  try {
    ...
    const documents = await DocumentOutsider.find({ _id: { $in: documentIds } });
    ...
    const result = await DocumentOutsider.deleteMany({ _id: { $in: documentIds } });
