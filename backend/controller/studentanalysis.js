...
export const getStudentAnalytics = async (req, res) => {
  try {
    ...
    if (cgpa && cgpa !== 'All') filter.cgpa = { $gt: parseFloat(cgpa.replace('> ', '')) };
    if (gender && gender !== 'All') filter.gender = gender;
    if (debarred && debarred !== 'All') filter.debarred = debarred === 'true';
    if (active_backlogs && active_backlogs !== 'All') filter.active_backlogs = active_backlogs === 'true';
    if (backlogs_history && backlogs_history !== 'All') filter.backlogs_history = backlogs_history === 'true';
    ...
    if (internshipstatus && internshipstatus !== 'All') filter.internshipstatus = internshipstatus;
    ...
    const studentsAnalytics = await Promise.all(
      students.map(async student => {
        try {
          ...
          const courseDurations = { "B.Tech": 4, "M.Tech": 2, "B.Sc.-B.Ed.": 4, "MBA": 2, "M.Sc.": 2 };
          ...
          for (const job of jobProfiles) {
            ...
            job.Hiring_Workflow?.forEach(step => {
              const type = (step.step_type || 'others').toLowerCase().replace(/\s+/g, '');
          ...
          Object.values(data.assessments).forEach(as => {
            if (as.total > 0) {
              as.successRate = ((as.shortlisted / as.total) * 100).toFixed(2) + '%';
            }
          });
          ...
          console.error(`Error processing student ${student.rollno}:`, err.message);
          return { _id: student._id, rollno: student.rollno, name: student.name || 'Unknown', error: err.message };
        }
      })
    );
